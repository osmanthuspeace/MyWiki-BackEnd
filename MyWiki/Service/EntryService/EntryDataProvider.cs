...
public class EntryDataProvider(WikiContext context) : IEntryDataProvider
{
    ...
    public async Task<EntryDto> GetEntryById(int id)
    {
        ...
        if (entry == null)
            throw new InvalidDataException("There is no entry for this id");
        return new EntryDto
        {
            Id = entry.EntryId,
            Title = entry.Title,
    ...
    public async Task<List<EntryDto>> GetEntriesByTitle(string title, int page, int pageSize)
    {
        ...
        if (title.IsNullOrEmpty())
            throw new InvalidOperationException("please specify the title");
        var entries = await context
            .Entries.Where(entry => entry.Title.Contains(title))
            .OrderBy(e => e)
            .Include(e => e.Tags)
    ...
    // 通过Tags获取词条
    public async Task<ICollection<EntryDto>> GetEntriesByTags(
    ...
    )
    {
        ...
        if (tagNames.IsNullOrEmpty())
            throw new Exception("please specify the tags");
        var entries = await context
            .Entries.Where(entry => entry.Tags.Any(tag => tagNames!.Contains(tag.TagName)))
            ...
            .ToListAsync();
        if (entries.Count == 0)
            throw new Exception("There is no entry under these tags");
        return entries
    ...
    // 通过Category获取词条
    public async Task<ICollection<EntryDto>> GetEntriesByCategory(
        string categoryName,
        int page,
        int pageSize
    )
    {
        ...
        if (categoryName.IsNullOrEmpty())
            throw new Exception("please specify the category");

        var existCategory = await context
        ...
        var entries = await context
            .Entries.Where(entry => entry.Category!.CategoryName == categoryName)
            .Include(e => e.Tags)
            // .Include(e=>e.Category)
            .Skip(skipCount)
            .Take(pageSize)
            .ToListAsync();
        if (entries.Count == 0)
            throw new Exception("There are no entries under this category");
        return entries
            .Select(entry => new EntryDto
            {
                Id = entry.EntryId,
    ...
    {
        //检查是否有已存在的词条
        var previousEntry = await context.Entries.FirstOrDefaultAsync(e =>
            e.Title == entryDto.Title
        );

        //有，就删掉（覆盖）
        if (previousEntry != null)
            throw new Exception("The entry has already exist!");
        ...
        var existingTags = await context
            .Tags.Where(tagInDataBase => tagsInDto.Contains(tagInDataBase.TagName))
        ...
        foreach (var tagInDto in tagsInDto)
        {
            ...
            var newTag = new Tag { TagName = tagInDto };
    ...
    {
        var entry = await context
            .Entries.Include(e => e.Tags)
            .FirstOrDefaultAsync(e => e.EntryId == entryDto.Id);
        if (entry == null)
            throw new Exception("The Entry isn't exist!");
    ...
    public async Task<string> DeleteEntry(string title)
    {
        ...
        if (entry == null)
            throw new Exception("There is no entry with this title");
    ...
    public int GetEntryTotal()
    {
        ...
        if (total == 0)
            throw new Exception("There is no entry in the database");
        return total;
    }
    ...
    {
        var tags = context.Tags.Select(t => t).OrderBy(t => t.TagId).ToListAsync();
        if (tags == null)
    ...
    {
        var existCategory = await context.Categories.FirstOrDefaultAsync(c =>
            c.CategoryName == categoryName
        );
        if (existCategory != null)
            return existCategory;
        var newCategory = new Category { CategoryName = categoryName, Description = "" };
