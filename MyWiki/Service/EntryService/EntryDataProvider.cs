...
public class EntryDataProvider(WikiContext context) : IEntryDataProvider
{
    ...
    // 通过Tags获取词条
    public async Task<ICollection<EntryDto>> GetEntriesByTags([FromQuery] List<string>? tagNames, int page,
        int pageSize)
    {
        var skipCount = (page - 1) * pageSize;
    ...
    public async Task<string> UpdateEntry(EntryDto entryDto)
    {
        ...
        entry.Tags.Clear();
        foreach (var tag in updatedTags) entry.Tags.Add(tag); // 添加新的标签关联
        await context.SaveChangesAsync();
